...
def a_star(start: tuple, goal: tuple, h: callable, heights: np.ndarray):
    ...
    _start = Node(fscore=heights.shape[0] * heights.shape[1], position=start)
    open_set = {}
    open_set_heap = []

    open_set[_start.position] = _start
    heapq.heappush(open_set_heap, _start)
    ...
    while open_set:  # is not empty:
        ...
        node = heapq.heappop(open_set_heap)
        ...
        for neighbor in get_neighbors(current):
            ...
            if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                ...
                if neighbor not in open_set:
                    neighbor_node = Node(fscore=f_score[neighbor], position=neighbor)
                    heapq.heappush(open_set_heap, neighbor_node)
                    open_set[neighbor] = neighbor_node
                else:
                    open_set[neighbor].fscore = f_score[neighbor]
...
grid[S] = 0
grid[E] = ord("z")-ord("a")
S_ALL = list(zip(*np.where(grid == 0)))

current_min = float("inf")

for start in S_ALL:
    res = a_star(start, E, lambda x, y: abs(x[1] - x[0]) + abs(y[1] - y[0]), grid)
    if not res:
        continue
    if len(res) < current_min:
        current_min = len(res)

print(current_min, time() - s)
