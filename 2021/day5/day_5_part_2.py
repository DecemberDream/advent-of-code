...
def covered_points(start, end, all_points):
    ...
    if np.abs(start[0] - end[0]) == np.abs(start[1] - end[1]):
        ...
        r_x = range(start[0], end[0] + step_x, step_x)
        r_y = range(start[1], end[1] + step_y, step_y)
        for i in range(len(r_x)):
            all_points.append([r_x[i], r_y[i]])
    if min_x == max_x:
...
for i in range(len(mask)):
    if np.abs(x1_y1[i, 0] - x2_y2[i, 0]) == np.abs(x1_y1[i, 1] - x2_y2[i, 1]):
        mask[i, 0] = True
        mask[i, 1] = True
